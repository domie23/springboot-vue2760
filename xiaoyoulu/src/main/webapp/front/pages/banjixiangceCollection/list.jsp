        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">班级相册收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">班级相册名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="banjixiangceName" id="banjixiangceName" placeholder="班级相册名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                         
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">班级相册类型</label>
                              <div class="layui-input-inline">
                                  <select name="banjixiangceTypes" id="banjixiangceTypes" lay-filter="banjixiangceTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in banjixiangceTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="gallery">
                        <div class="row">
                            <div class="col-md-3" v-for="(item,index) in dataList" >
                                <div class="galry-grid grid-top-rgt">
                                    <i @click="deleteBanjixiangceCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                    <a href="javascript:return false;"  @click="jumpCheck('../banjixiangce/detail.jsp?id='+item.banjixiangceId , item.banjixiangceDelete == null?'':item.banjixiangceDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                        <img class="img-responsive zoom-img" :src="item.banjixiangcePhoto?item.banjixiangcePhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                        <div class="w3agile-text">
                                            <h5>
                                                <span>{{item.banjixiangceName}}</span>
                                                <br/>
                                                <span v-if="item.banjixiangceNewMoney">
                                                    {{item.banjixiangceNewMoney}} RMB
                                                </span>
                                                <span v-else>
                                                    {{item.banjixiangceValue}}
                                                </span>
                                            </h5>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            banjixiangceCollectionTypesList : [],
        // 级联表字典表数据
            banjixiangceTypesList : [],
            banjixiangceCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteBanjixiangceCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('banjixiangceCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function banjixiangceCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=banjixiangce_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.banjixiangceCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function banjixiangceTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=banjixiangce_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.banjixiangceTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#banjixiangceName').val() != null && jquery('#banjixiangceName').val() != "" && jquery('#banjixiangceName').val() != "null" ) {
                param['banjixiangceName'] = jquery('#banjixiangceName').val();
            }
                                         
            var banjixiangceTypes = $("#banjixiangceTypes").find("option:selected").val();
            if (banjixiangceTypes != null && banjixiangceTypes != "" && banjixiangceTypes != "null") {
                            param['banjixiangceTypes'] = banjixiangceTypes;
                        }
                                                                                
            // 当前表
                                                
            //收藏只能查类型为1的
            param['banjixiangceCollectionTypes'] = 1;

            // 获取列表数据
            http.request('banjixiangceCollection/page', 'get', param, function(res) {
